...
public class CuriosScreen extends AbstractRecipeBookScreen<CuriosMenu>
  ...
  private final List<GuiEventListener> curioWidgets = new ArrayList<>();
  ...
  public void updateRenderButtons() {
    Iterator<GuiEventListener> iterator = this.curioWidgets.iterator();

    while (iterator.hasNext()) {
      this.removeWidget(iterator.next());
      iterator.remove();
    }
    this.panelWidth = this.menu.panelWidth;
    ...
    if (this.menu.hasCosmetics) {
      this.cosmeticButton =
          new CosmeticButton(this, this.leftPos + 17, this.topPos - 18, 20, 17);
      this.curioWidgets.add(this.addRenderableWidget(this.cosmeticButton));
    }

    if (this.menu.totalPages > 1) {
      this.nextPage =
          new PageButton(
              this, this.leftPos + 17, this.topPos + 2, 11, 12, PageButton.Type.NEXT);
      this.curioWidgets.add(this.addRenderableWidget(this.nextPage));
      this.prevPage =
          new PageButton(
              this, this.leftPos + 17, this.topPos + 2, 11, 12, PageButton.Type.PREVIOUS);
      this.curioWidgets.add(this.addRenderableWidget(this.prevPage));
    }

    for (Slot inventorySlot : this.menu.slots) {

      if (inventorySlot instanceof CurioSlot curioSlot && curioSlot.canToggleRender()) {
        this.curioWidgets.add(this.addRenderableWidget(
            new RenderButton(
                curioSlot,
                ...
                CURIO_INVENTORY,
                (button) ->
                    top.theillusivec4.curios.platform.fabric.CuriosFabricNetworking.sendToServer(
                        new CPacketToggleRender(
                            curioSlot.getIdentifier(), curioSlot.getSlotIndex())))));
      }
    }
  }
  ...
  public void render(@Nonnull GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
    ...
    for (GuiEventListener child : this.children()) {
      ...
      if (child instanceof RenderButton renderButton) {
  ...
  public boolean keyPressed(@Nonnull KeyEvent event) {
    ...
    if (super.keyPressed(event)) {
      return true;
    } else if (CuriosKeyMappings.OPEN_CURIOS_INVENTORY.matches(event)) {
  ...
  public boolean mouseScrolled(
    ...
    if (this.menu.totalPages > 1
        && p_94686_ < this.leftPos
        && p_94686_ > this.leftPos - this.panelWidth
        && p_94687_ > this.topPos
        && p_94687_ < this.topPos + this.imageHeight
        && scrollCooldown <= 0) {
      top.theillusivec4.curios.platform.fabric.CuriosFabricNetworking.sendToServer(
